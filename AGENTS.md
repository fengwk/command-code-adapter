...
- **Balance probes are staggered**: the scheduler refreshes every key's credits on one cadence, so
  ...
  multi-account signature. The first (cold) fetch and explicit refreshes stay immediate; concurrent cold
  requests coalesce into one fetch wave.
...
- **ID generation**: `generate_id(prefix, length)` and opaque deterministic admin key references via `api_key_id(key)` in `core/utils.py`.
- **Constants**: `core/constants.py` — `STREAMING_HEADERS`, `NPM_URL`, `NPM_CACHE_TTL`, `NPM_ERROR_BACKOFF`, `NPM_FETCH_HEADERS`, `KEY_CREDITS_CACHE_TTL`, `KEY_CREDITS_ERROR_BACKOFF`, `KEY_CREDITS_PROBE_SPREAD`, `KEY_COOLDOWN_BASE`, `KEY_COOLDOWN_MAX`, `KEY_CREDIT_COOLDOWN`, `KEY_FORBIDDEN_COOLDOWN`, `KEY_MAX_CONCURRENT_STREAMS`, `DISTRIBUTION_ROUND_ROBIN`/`DISTRIBUTION_FILL_FIRST` (`DISTRIBUTION_MODES`, `DEFAULT_DISTRIBUTION`, `normalize_distribution()`), `SESSION_AFFINITY_TTL`, `SESSION_AFFINITY_MAX_ENTRIES`, `PROJECT_SLUGS_PER_ACCOUNT_MIN/MAX`, `VERSION`. The three key cooldowns are also config fields (`CC_ADAPTER_KEY_COOLDOWN_BASE|MAX`, `CC_ADAPTER_KEY_CREDIT_COOLDOWN`) so deployments can tune them from the environment, and the distribution mode has `CC_ADAPTER_DISTRIBUTION`; `KEY_FORBIDDEN_COOLDOWN` and `KEY_MAX_CONCURRENT_STREAMS` are hardcoded disguise knobs.
...
- **Key scheduler**: `core/key_scheduler.py` — `KeyScheduler` replaces the old `KeyPool` (deleted). Owns per-key health (ok/cooling/disabled), credits, per-session sticky bindings and the first-sight distribution mode (`distribution` property, `set_distribution()`; `round-robin` by default, `fill-first` selectable). `export_state()` / `import_state()` carry retained-key health, credits, manual switches and affinity across panel client rebuilds; a newly added key deliberately invalidates snapshot freshness so the next concurrent requests share one cold fetch wave. `select(..., load=client.key_load)` applies the per-key concurrency cap.
...
- **First-sight distribution then stickiness**: a *new* stream (a session that is not bound yet, or a request without any
  session identity) is dealt by the configured mode: `round-robin` (the default) takes the next slot of the configured ring,
  `fill-first` always starts at the first usable key. The mode governs the fan-out of new sessions only — it never moves a bound
  session, and a saturated pool still returns the least loaded key — and `explicit` no longer selects it: the signal is only
  information about the caller's identity (client-provided vs. content anchor) and is logged. Either way the choice is bound to
  that session; without the binding a conversation would follow the head key's cooldown and back, so one conversation would keep
  showing up under two accounts.
- **Switching the mode**: `KeyScheduler.set_distribution()` (`POST`/`PUT /admin/api/config {"distribution": …}`, persisted as
